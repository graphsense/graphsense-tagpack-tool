    ...
    def insert_tagpack(self, tagpack, batch=1000):
        ...
        for tag in tagpack.tags:
            ...
            if len(tag_data) > batch:
                execute_batch(self.cursor, "INSERT INTO address (currency, address) VALUES (%s, %s) ON CONFLICT DO NOTHING", address_data)
                execute_batch(self.cursor, "INSERT INTO tag (label, source, category, abuse, address, currency, tagpack) VALUES (%s, %s, %s, %s, %s, %s, %s)", tag_data)
...
def _get_tag(tag, tagpack_id):
    return (tag.all_fields.get('label'), tag.all_fields.get('source'), tag.all_fields.get('category', None),
            tag.all_fields.get('abuse', None), tag.all_fields.get('address'), tag.all_fields.get('currency'), tagpack_id)
